...
function definePage(options) {
  const { tab = 0, inlineTab = false, data = {}, onLoad, onShow, ...rest } = options;
  ...
  return Page(
    Object.assign({}, rest, {
      ...
      goto(e) {
        const { url, tab: toTab } = e.currentTarget.dataset;
        if (!url) return;
      ...
      back() {
        ...
        else wx.switchTab({ url: '/pages/mainline/index' });
      },

      /** 演示态提示：没接后端的按钮点了要有回响，别让老板以为卡住了 */
      toast(e) {
        const msg = (e.currentTarget.dataset.msg || '演示版：该动作已记录').toString();
        wx.showToast({ title: msg, icon: 'none', duration: 1600 });
      },
    })
  );
}

module.exports = { definePage, demo };
