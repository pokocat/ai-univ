...
import { Block, View, Canvas, Text, Button } from '@tarojs/components'
...
const RULES = [
  {
    icon: 'award',
    title: '邀请奖励',
    desc: '好友经你的邀请码注册成功即发放成长值奖励（以页面展示为准）',
  },
  {
    icon: 'link',
    title: '关系绑定',
    desc: '注册即自动绑定关系链，最多三级；绑定后不可更改',
  },
  {
    icon: 'shield',
    title: '奖励性质',
    desc: '奖励为成长值等虚拟权益，不涉及现金返利，平台不做返佣结算',
  },
]
  ...
  draw(invite, qrBase64) {
    Taro.createSelectorQuery()
      ...
      .exec((res) => {
        ...
        const dpr =
          (Taro.getWindowInfo ? Taro.getWindowInfo() : Taro.getSystemInfoSync())
            .pixelRatio || 2
        ...
        ctx.fillText(
          qrBase64 ? '长按识别右侧小程序码' : '注册时手动输入邀请码即可',
          42,
          boxY + 90
        )
        const finish = () =>
        ...
        const path = `${Taro.env.USER_DATA_PATH}/poster-qr.png`
        try {
      ...
      fail: (err) => {
        if (String(err.errMsg || '').indexOf('auth') >= 0) {
          Taro.showModal({
            title: '需要相册权限',
            content:
              '保存海报需要相册写入权限，可在「设置 → 主理人公社」中开启后重试。',
            confirmText: '去设置',
            success: (r) => {
...
class _C extends React.Component {
  render() {
    const { err, load, loading, cw, ch, drawn, rules } = this.data
    return (
      <View className="screen screen--bar">
        <View className="ambience"></View>
        <View className="layer">
          <UiSubhead title="邀请海报" home="/pages/invite/index"></UiSubhead>
          {err && (
            <UiErrbar msg={err} retrying={load} onRetry={this.load}></UiErrbar>
          )}
          {loading ? (
            ...
            <Block>
                ...
                {rules.map((item, index) => {
                  return (
                    <View key={item.title} className="row-item">
                      ...
                      <View className="f-1">
                        <Text className="rule-t">{item.title}</Text>
                        <Text className="t-tiny mt-6 block">{item.desc}</Text>
                      </View>
                    </View>
                  )
                })}
              ...
              <View className="box box--sunk gap">
                ...
                >
                  ...
                  <Text className="t-sec fw-600">关于海报</Text>
                </View>
                <Text className="t-tiny mt-6 block">
                  海报在本机绘制，包含你的邀请码与小程序码。小程序码未生成时海报仍可保存，
                  好友可以手动输入邀请码——不会给你一张扫不出来的图。
                </Text>
              </View>
            </Block>
          )}
        ...
        <View className="action-bar">
          <View className="action-row">
            <View className="cta cta--primary" onClick={this.savePoster}>
