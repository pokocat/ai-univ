  ...
  share.withShare({
    ...
    async load() {
      ...
      try {
        ...
        const [me, growth, courses, announcements, diagnosis, invite] =
          await Promise.all([
            api.getMe(),
            api.getGrowth(5),
        ...
        this.setData({
          ...
          inviteAward:
            invite && invite.inviteAward != null
              ? money(invite.inviteAward)
              : null,
          unread: (announcements && announcements.unread) || 0,
        })
        // 服务提醒依赖另外两个接口，单独拉：它是辅助区块，慢一点不该拖住首屏
        this.setData({
          reminder: await this.buildReminder(diagnosis),
        })
        const tabBar =
          typeof this.getTabBar === 'function' ? this.getTabBar() : null
    ...
    buildInsights(diagnosis, announcements, me) {
      ...
      if (identity.valid_until) {
        const days = Math.ceil(
          (new Date(identity.valid_until).getTime() - Date.now()) / 86400000
        )
        if (days >= 0 && days <= 30) {
          out.push({
            eyebrow: '会员到期提醒',
    ...
    decorateCourse(c) {
      const at = c.scheduled_at
        ? new Date(
            String(c.scheduled_at).replace(/-/g, '/').replace(/\..*$/, '')
          )
        : null
    ...
    async buildReminder(diagnosis) {
      try {
        const [group, tickets] = await Promise.all([
          api.getMyGroup(),
          api.getTickets(),
        ])
        ...
        if (teacher) {
          return {
            ...
            desc:
              (teacher.role || '社群服务') +
              (teacher.service_region ? ' · ' + teacher.service_region : '') +
              '，有问题随时找他',
            actionText: '立即联系',
            url: '/pages/advisor/index',
          }
        ...
        <View className="layer">
          ...
          {loading ? (
            ...
            <Block>
                      ...
                      <SwiperItem key={item.title}>
                        <View className="insight">
                          ...
                          <View className="insight-body">
                            <View className="f-1">
                              <View className="title-grad insight-title">
                                {item.title}
                              </View>
                              <Text className="lead insight-text">
                                {item.body}
                              </Text>
                              <View
              ...
              {growth && (
                <View className="card card-pad gap">
                  <View className="f-between">
                    <Text className="col-h">会员成长进度</Text>
                    <View
                    ...
                    >
                      <Text>成长值明细</Text>
                      <UiIcon name="chev" size={24} color="ink-800"></UiIcon>
                    </View>
                  </View>
                  <View className="growth mt-10">
                    <UiArt
                      kind="medallion"
                      w={92}
                      glyph={growth.glyph}
                    ...
                    <View className="f-1">
                      ...
                      >
                        <Text className="title-grad growth-lv mono">
                          {growth.levelText}
                        </Text>
                        <Text className="t-h3">
                      ...
                      <View className="prog-track mt-10">
                        <View
                          className="prog-fill"
                      ...
                      <View className="f-between mt-10">
                        <Text className="t-sec">
                          成长值
                          <Text className="mono fw-700 t-ink">
                            {growth.growthText}
                          </Text>
                          {growth.nextText && (
                            <Text>{'/ ' + growth.nextText}</Text>
                          )}
                        </Text>
                        {!growth.topLevel ? (
                          <View className="ta-r">
                            <Text className="t-tiny">距升级还差</Text>
                            <Text className="t-tiny mono t-pulse">
                              {growth.remainText}
                            </Text>
                          </View>
                        ) : (
              ...
              <View className="two-col gap">
                {quick.map((item, index) => {
                  return (
                    ...
                    >
                      <View className="f-1">
                        <Text className="quick-t">{item.title}</Text>
                        <Text className="quick-d">{item.desc}</Text>
                      </View>
                      <View
                        className="f"
                        style={{
              ...
              {/*  ④ 近期课程 / 服务提醒  */}
              <View className="two-col gap">
                <View className="card card-pad col">
                  <View className="f-between">
                    <Text className="col-h">近期课程</Text>
                    <View
                      className="link-trail"
                      data-url="/pages/training/index"
                      onClick={this.go}
                    >
                      <Text className="t-tiny">全部</Text>
                  ...
                  {course ? (
                        ...
                        >
                          {course.live && (
                            <View className="tag tag--pink">
                              <View className="live-dot"></View>
                              <Text>直播中</Text>
                            </View>
                          )}
                          <Text className="course-t">{course.title}</Text>
                        </View>
                        <Text className="t-tiny mt-6 block">
                          {course.speaker +
                            (course.speaker_title
                              ? ' · ' + course.speaker_title
                              : '')}
                        </Text>
                        <Text className="t-tiny mono mt-2 block">
                          {course.whenText}
                        </Text>
                      </View>
                      <View className="hr mt-10"></View>
                      <View className="f-between mt-10">
                        {course.countdown ? (
                          <Text className="t-tiny">
                            距开始
                            <Text className="mono t-ok fw-700">
                              {course.countdown}
                            </Text>
                          </Text>
                        ) : (
                          <Text className="t-tiny">{course.status}</Text>
                        )}
                        <View
                          className="btn-sm"
                          data-url={'/pages/course/index?id=' + course.id}
                          onClick={this.go}
                        >
                          {course.actionText}
                        </View>
                    ...
                    <View className="blank">
                      <View className="blank-mark">
                        <UiIcon name="cap" size={45} color="signal"></UiIcon>
                      </View>
                      <Text className="blank-title">还没有排期课程</Text>
                      <Text className="blank-hint">
                        开课后会在这里提醒你，也可以先看看培训服务里的往期回放。
                      </Text>
                    </View>
                  )}
                </View>
                <View className="card card-pad col">
                  <View className="f-between">
                    <Text className="col-h">服务提醒</Text>
                    <View
                      className="link-trail"
                  ...
                  {reminder ? (
                    <Block>
                      ...
                      >
                        <UiIcon
                          ...
                          size={45}
                          color="pulse"
                          extStyle="margin-top:0.1rem;"
                        ></UiIcon>
                        <View className="f-1">
                          <Text className="reminder-t">{reminder.title}</Text>
                          <Text className="t-tiny mt-6 block">
                            {reminder.desc}
                          </Text>
                        </View>
                      </View>
                      <View className="reminder-cta">
                        <View
                          className="btn-grad btn-grad--full"
                    ...
                    <View className="mt-10">
                      <View className="skeleton skeleton-line"></View>
                      <View className="skeleton skeleton-line skeleton-line--body"></View>
                    </View>
                  )}
                </View>
              ...
              {/*  ⑤ 邀请横幅  */}
              <View
                className="card card--signal invite gap"
                data-url="/pages/invite/index"
                onClick={this.go}
              >
                <UiArt kind="trophy" w={80}></UiArt>
                <View className="f-1">
                  <Text className="invite-t">邀请好友 · 共创价值</Text>
                  {inviteAward ? (
                    <Text className="invite-d">
                      邀请 1 位好友可得
                      <Text className="mono invite-num">{inviteAward}</Text>
                      成长值
                    </Text>
                  ) : (
                    <Text className="invite-d">
