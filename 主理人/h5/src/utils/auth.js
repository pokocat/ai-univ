...
function captureInviteCode(options) {
  ...
  try {
    fromScene =
      typeof q.scene === 'string' ? decodeURIComponent(q.scene) : undefined
  } catch (e) {
    fromScene = undefined // scene 非法编码不应中断启动
  }
  const code =
    fromQuery ||
    (fromScene && fromScene.indexOf('FLM-') === 0 ? fromScene : undefined)
  if (code) {
...
function captureScanTicket(options) {
  ...
  if (ticket) {
    Taro.setStorageSync(SCAN_TICKET_KEY, ticket)
...
function getPendingScanTicket() {
  const t = Taro.getStorageSync(SCAN_TICKET_KEY)
  return isScanTicket(t) ? t : null
}
function clearPendingScanTicket() {
  Taro.removeStorageSync(SCAN_TICKET_KEY)
}
function getToken() {
 ...
 */
function hasConsent() {
  if (Taro.getStorageSync(CONSENT_KEY)) return true
...
async function doLogin() {
  ...
  Taro.setStorageSync(TOKEN_KEY, resp.data.token)
  Taro.setStorageSync(MEMBER_KEY, resp.data.memberNo)
  log.info('login ok', resp.data.memberNo)
  return {
    memberNo: resp.data.memberNo,
  }
}
...
function adoptToken(token, memberNo) {
  ...
  if (memberNo) Taro.setStorageSync(MEMBER_KEY, memberNo)
  inflight = null
