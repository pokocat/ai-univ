...
async function goGroupFallback(reason) {
  const result = await Taro.showModal({
    ...
    content:
      reason ||
      '微信客服暂未接通。你可以先到「我的社群」查看入群入口，并在群里联系服务老师。',
    confirmText: '去我的社群',
    cancelText: '稍后再说',
  })
  if (result.confirm) {
...
async function open() {
  try {
    ...
    if (!Taro.openCustomerServiceChat) {
      await goGroupFallback(
        '当前微信版本不支持打开微信客服，请升级微信；也可以先去「我的社群」联系服务老师。'
      )
      return
    }
    Taro.openCustomerServiceChat({
      ...
      fail: async (err) => {
        log.warn(
          'open customer service failed',
          err && err.errMsg ? err.errMsg : 'unknown'
        )
        await goGroupFallback(
          '微信客服打开失败。你可以先到「我的社群」查看入群入口，并在群里联系服务老师。'
        )
      },
    })
  } catch (e) {
    log.warn(
      'customer service config unavailable',
      e && e.message ? e.message : 'unknown'
    )
    await goGroupFallback(
      '客服配置暂时无法读取。你可以先到「我的社群」联系服务老师。'
    )
  }
}
module.exports = {
