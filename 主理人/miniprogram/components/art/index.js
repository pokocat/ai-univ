 ...
 */
const KINDS = ["gem", "medallion", "check", "aigem", "aicube", "megaphone", "trophy"];
...
Component({
  properties: {
    ...
    kind: { type: String, value: "gem" },
    /** 宽高（rpx）；h 传 0 时按 BOX 的比例推算 */
    w: { type: Number, value: 210 },
  ...
  data: { box: "", src: "", glyphText: "", glyphSize: 0, overlayIcon: "", iconSize: 0 },
  observers: {
  ...
  methods: {
    render() {
      ...
      if (KINDS.indexOf(kind) < 0) {
        ...
        this.setData({ box: "display:none", src: "" });
        return;
      }
      const [vw, vh] = BOX[kind];
      const height = h || Math.round((w * vh) / vw);
      const box = w ? `width:${w}rpx;height:${height}rpx;` : "width:100%;height:100%;";
      const text = check ? "" : String(glyph || "");
      this.setData({
        box: box + "flex-shrink:0;" + (extStyle || ""),
        src: "/assets/art/" + kind + ".png",
        glyphText: text,
        glyphSize: Math.round(w * (text.length > 1 ? GLYPH_RATIO_MULTI : GLYPH_RATIO_SINGLE)),
        // check 这个 kind 的素材里已经画了对勾，所以只有「gem 上叠 check」才需要图标层
        overlayIcon: text ? "" : (check && kind !== "check" ? "check" : icon || ""),
