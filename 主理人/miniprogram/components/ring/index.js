 ...
 */
const { resolve } = require("../../utils/icons");

Component({
  properties: {
    /** 0-100 */
    pct: { type: Number, value: 0 },
    /** 直径（rpx） */
    size: { type: Number, value: 100 },
    /** 环色：token 名或 # 字面值 */
    color: { type: String, value: "signal-600" },
    /** 环粗（相对 viewBox 的 58 基准，默认 4） */
    weight: { type: Number, value: 4 },
    /** 环内文字（维度名或百分比） */
    label: { type: String, value: "" },
    /** 环内文字字号（viewBox 单位） */
    labelSize: { type: Number, value: 9 },
    labelColor: { type: String, value: "ink-600" },
  ...
  methods: {
    render() {
      const { pct, size, color, weight, label, labelSize, labelColor, extStyle } = this.data;
      const r = 22;
      const c = 2 * Math.PI * r;
      ...
      const text = label
        ? `<text x="29" y="${29 + labelSize / 3}" text-anchor="middle" font-size="${labelSize}" ` +
