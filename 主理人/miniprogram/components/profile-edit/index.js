...
Component({
  properties: {
    ...
    title: { type: String, value: "完善资料" },
    subtitle: { type: String, value: "" },
    /** 首次注册登录为 true：手机号未验证时不允许完成。 */
    phoneRequired: { type: Boolean, value: false },
    /** 首次注册登录为 true：默认昵称或空昵称不允许完成。 */
    nameRequired: { type: Boolean, value: false },
    /** 已绑定手机号的脱敏展示；空串表示尚未绑定。 */
    phoneText: { type: String, value: "" },
  },
  ...
  lifetimes: {
    attached() {
      this.resetProfile();
      this.setData({ phoneDisplay: this.data.phoneText || "" });
  ...
  methods: {
     ...
     */
    computeSubmitLabel() {
      if (this.data.phoneRequired && !this.data.phoneDisplay) return "请先验证手机号";
    ...
    refreshPrivacySetting() {
      if (!wx.getPrivacySetting) {
        this.setData({ privacyReady: true, privacyAgreed: true });
        return;
      }
      wx.getPrivacySetting({
        ...
        fail: () => this.setData({ privacyReady: true, privacyAgreed: false }),
      });
    },
    ...
    onAgreePrivacy() {
      this.setData({ privacyReady: true, privacyAgreed: true, phoneScopeError: "" });
      wx.showToast({ title: "请继续验证手机号", icon: "none" });
    },
    ...
    resetProfile() {
      ...
      this.setData({
        tempAvatar: "",
        avatarSrc: this.data.avatarPath ? `${base()}${this.data.avatarPath}` : "",
    ...
    async onPhone(e) {
      ...
      try {
        ...
        this.setData({ phoneDisplay, canSave: true, phoneScopeError: "" });
        this.triggerEvent("phonebound", { phoneText: phoneDisplay });
    ...
    onChooseAvatar(e) {
      ...
      this.setData({ tempAvatar: url, canSave: true });
      if (!this.data.privacyAgreed) this.setData({ privacyAgreed: true, privacyReady: true });
    },
    ...
    async save() {
      ...
      const phoneCompleted = this.data.phoneRequired && this.data.phoneDisplay && this.data.canSave;
