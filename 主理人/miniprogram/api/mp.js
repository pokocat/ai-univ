...
module.exports = {
  ...
  getMe: () => request("/mp/me"),
  /** 站内消息中心（后台状态变化必达） */
  getNotifications: unreadOnly => request("/mp/notifications", { data: { unreadOnly: !!unreadOnly } }),
  getNotificationTemplates: () => request("/mp/notification-templates"),
  recordNotificationSubscriptions: results =>
    request("/mp/notification-subscriptions", { method: "POST", data: { results } }),
  readNotification: id => request(`/mp/notifications/${id}/read`, { method: "POST" }),
  readAllNotifications: () => request("/mp/notifications/read-all", { method: "POST" }),
  ...
  createOrder: (planCode, key) =>
    request("/mp/orders", {
      method: "POST",
      data: { planCode, channel: platformChannel() },
  ...
  closeOrder: orderNo => request(`/mp/orders/${orderNo}/close`, { method: "POST" }),
  getMyGroup: () => request("/mp/my-group"),
  getCourses: () => request("/mp/courses"),
  getCourseWatchCode: id => request(`/mp/courses/${id}/watch-code`, { method: "POST" }),
   ...
   */
  uploadAvatar: tempFilePath => upload("/mp/profile/avatar", tempFilePath, "file"),
  /** 申请注销账号（建「账号问题」工单人工核验；不即时删档，理由见后端注释） */
  requestAccountDeletion: reason => request("/mp/account/deletion", { method: "POST", data: { reason } }),
  ...
  applyAgent: (identity, reason) =>
    request("/mp/agent/application", { method: "POST", data: { identity, reason } }),
  ...
  getGrowth: (limit) => request("/mp/growth", { data: { limit: limit || 20 } }),
  /** 权益清单；onlyAvailable=true 只返回当前可用（会员卡宫格用） */
  getBenefits: onlyAvailable => request("/mp/benefits", { data: { onlyAvailable: !!onlyAvailable } }),
  ...
  getAnnouncements: (category, limit) =>
    request("/mp/announcements", { data: { category: category || "", limit: limit || 20 } }),
  getAnnouncement: annNo => request(`/mp/announcements/${annNo}`),
  readAnnouncement: annNo => request(`/mp/announcements/${annNo}/read`, { method: "POST" }),

  // ── 课程与课件（设计稿 08·17·29） ──

  getCourseSessions: courseType =>
    request("/mp/course-sessions", { data: { courseType: courseType || "" } }),
  getCourseSession: id => request(`/mp/course-sessions/${id}`),
  enrollCourse: id => request(`/mp/course-sessions/${id}/enroll`, { method: "POST" }),
  saveChapterProgress: (chapterId, watchedSeconds) =>
    request(`/mp/course-chapters/${chapterId}/progress`, { method: "POST", data: { watchedSeconds } }),
  recordDownload: materialId =>
    request(`/mp/course-materials/${materialId}/download`, { method: "POST" }),

  // ── 班级活动（设计稿 06） ──

  getActivities: () => request("/mp/activities"),
  signUpActivity: activityNo => request("/mp/activities/signup", { method: "POST", data: { activityNo } }),
  ...
  createDiagnosisBooking: (payload, key) =>
    request("/mp/diagnosis/bookings", {
      method: "POST",
      data: payload,
      headers: { "Idempotency-Key": key || idemKey("mpdiag") },
    }),
  getDiagnosisBooking: bookingNo => request(`/mp/diagnosis/bookings/${bookingNo}`),
  cancelDiagnosisBooking: (bookingNo, reason) =>
    request(`/mp/diagnosis/bookings/${bookingNo}/cancel`, { method: "POST", data: { reason } }),

  // ── 服务工单（设计稿 15·28） ──

  getTicketTypes: () => request("/mp/ticket-types"),
  getTickets: status => request("/mp/tickets", { data: { status: status || "" } }),
  ...
  createTicket: (ticketType, description, attachments, key) =>
    request("/mp/tickets", {
      method: "POST",
      data: { ticketType, description, attachments },
      headers: { "Idempotency-Key": key || idemKey("mpticket") },
    }),

  // ── 服务评价（设计稿 24） ──

  getReviewSubject: (targetType, targetRef) =>
    request("/mp/reviews", { data: { targetType, targetRef } }),
  submitReview: payload => request("/mp/reviews", { method: "POST", data: payload }),
  ...
  applyRefund: (orderNo, reason, note, key) =>
    request("/mp/refunds", {
      method: "POST",
      data: { orderNo, reason, note },
      headers: { "Idempotency-Key": key || idemKey("mprefund") },
    }),
   ...
   */
  scanLoginConfirm: ticket => request("/mp/scan-login/confirm", { method: "POST", data: { ticket } }),
};
