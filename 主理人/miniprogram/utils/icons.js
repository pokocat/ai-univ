 ...
 */
const { PATHS } = require("./icon-paths");
...
module.exports = { PATHS, ALIASES, TOKENS, resolve, svgUri };

/** token 名 / # 字面值 → 字面色值；未知名字回落到弱化文字色 */
function resolve(color) {
  return String(color).charAt(0) === "#" ? color : TOKENS[color] || TOKENS["ink-600"];
}
...
function svgUri(name, opts) {
  ...
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${o.fill ? hex : "none"}" ` +
