...
async function prefetch(force) {
  ...
  loading = api.getNotificationTemplates()
    .then(data => {
...
function request(keys) {
  ...
  if (!templates.length || !wx.requestSubscribeMessage) return Promise.resolve({ accepted: 0, configured: templates.length });
    ...
    wx.requestSubscribeMessage({
      ...
      success: async result => {
        ...
        try {
          ...
          resolve({ accepted: saved.accepted || 0, configured: templates.length });
        } catch (err) {
          log.warn("record notification subscription failed", err.message);
          resolve({ accepted: 0, configured: templates.length });
        }
      },
      fail: err => {
...
module.exports = { prefetch, request, configured, clear };
