 ...
 */
const IMAGE = "/assets/share.jpg";
const TIMELINE_IMAGE = "/assets/share-timeline.jpg";
const LANDING = "/pages/index/index";

/** 当前可用的邀请码（页面调 setShareCode 写入；未登录/未拉到时为空） */
function currentCode() {
  try {
    return getApp().globalData.inviteCode || "";
  } catch (e) {
...
const mixin = {
  ...
  setShareCode(code) {
    if (code) getApp().globalData.inviteCode = code;
  },

  onShareAppMessage() {
    return { title: TITLE, path: pathWithCode(currentCode()), imageUrl: IMAGE };
  },
...
module.exports = { withShare, mixin, pathWithCode, TITLE, TIMELINE_TITLE, IMAGE, LANDING };
